...
void *serve_client(void *socket) {
    ...
    if ((ret = recv(* (int*)socket, buff, MAXBUFF-1, 0)) < 0) {
        //printf("%s ", prog);
        perror("recv()");
       return (void *) -1;
    }
    ...
    int ret2;
    if ((ret2 = send(* (int*)socket, "Acknowledged", 12, 0)) < 0) {
        //printf("%s ", prog);
        perror("send()");
        return (void *) -1;
    }
    ...
    if (shutdown(* (int*)socket, SHUT_RDWR) < 0) {
        //printf("%s ", prog);
        perror("close(clientd)");
        return (void *) -1;
    }
    if ((ret = close(* (int*)socket)) < 0) {
        //printf("%s ", prog);
        perror("close(clientd)");
        return (void *) -1;
    }
...
int main(int argc, char **argv) {
  ...
  if ((clientd = accept(serverd, (struct sockaddr*) &ca, &size)) < 0) {
    ...
    if ((ret = pthread_create(&id, NULL, serve_client, &clientd) < 0)) {
        perror("pthread creation failed");
        return -1;
    }
    //int returnvalue = serve_client(clientd);
